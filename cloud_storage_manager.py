...
class CloudStorageManager:
    ...
    def _build_audio_path(
        self, project_id: str, language: str, character: str, filename: str
    ) -> str:
    ...
    def upload_audio(
        self, local_path: str, project_id: str, language: str, character: str
    ) -> str:
        """Upload an audio file to the specified path structure."""
        try:
            filename = os.path.basename(local_path)
            blob_path = self._build_audio_path(
                project_id, language, character, filename
            )
    ...
    def upload_model(
        ...
        try:
            filename = os.path.basename(local_path)
            blob_path = self._build_model_path(
                project_id, language, character, model_version, filename
            )
            blob = self.bucket.blob(blob_path)
            blob.upload_from_filename(local_path)
            return blob_path
    ...
    def list_audio_files(
        self, project_id: str, language: str, character: str
    ) -> List[str]:
