...
def run_inference(
    ...
    best_pth = next(
        (
            ...
            if f.endswith(".pth") and model_name in f and "_best_model" in f
        ),
        None,
    )
    if best_pth is not None:
        ...
        pth_path = next(
            (f for f in files_in_folder if f.endswith(".pth") and model_name in f), None
        )
    ...
    cmd = [
        ...
        str(reverb_width),
        "--reverb_freeze_mode",
        str(reverb_freeze_mode),
        "--pitch_shift_semitones",
        str(pitch_shift_semitones),
        "--limiter_threshold",
        str(limiter_threshold),
        "--limiter_release_time",
        str(limiter_release_time),
        "--gain_db",
        str(gain_db),
        "--distortion_gain",
        str(distortion_gain),
        ...
        str(chorus_depth),
        "--chorus_center_delay",
        str(chorus_center_delay),
        "--chorus_feedback",
        str(chorus_feedback),
        "--chorus_mix",
        str(chorus_mix),
        "--bitcrush_bit_depth",
        str(bitcrush_bit_depth),
        "--clipping_threshold",
        str(clipping_threshold),
        "--compressor_threshold",
        str(compressor_threshold),
        "--compressor_ratio",
        str(compressor_ratio),
        "--compressor_attack",
        str(compressor_attack),
        "--compressor_release",
        str(compressor_release),
        "--delay_seconds",
        str(delay_seconds),
        "--delay_feedback",
        str(delay_feedback),
